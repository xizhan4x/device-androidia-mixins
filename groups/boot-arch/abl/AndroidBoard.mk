...
BOARD_BLDR_UTILS_IMG := $(PRODUCT_OUT)/bldr_utils.img
BLDR_UTILS_DIR := $(PRODUCT_OUT)/bldr_utils

INSTALLED_RADIOIMAGE_TARGET += $(BOARD_BLDR_UTILS_IMG)

$(BLDR_UTILS_DIR):
	$(hide) rm -rf $(BLDR_UTILS_DIR)
	$(hide) mkdir -p $(BLDR_UTILS_DIR)

ifneq ($(BOARD_BOOTLOADER_PARTITION_SIZE),0)
$(BOARD_BLDR_UTILS_IMG): $(BLDR_UTILS_DIR) mkext2img ifwi_update kf4abl-$(TARGET_BUILD_VARIANT)
	$(hide) $(ACP) -f $(BOARD_BLDR_SECOND_STAGE_BIN) $(BLDR_UTILS_DIR)/fastboot.bin
	mkext2img  $(BOARD_BLDR_UTILS_IMG) $(BOARD_BOOTLOADER_BLOCK_SIZE) $(BOARD_BOOTLOADER_PARTITION_SIZE) $(BLDR_UTILS_DIR)
	echo "Bldr utils image successfully generated $(BOARD_BLDR_UTILS_IMG)"
else
	$(ACP) -f $(ABL_PREBUILT_PATH)/bldr_utils.img $(BOARD_BLDR_UTILS_IMG)
endif

droidcore: $(BOARD_BLDR_UTILS_IMG)
